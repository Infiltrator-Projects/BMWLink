...
{
    GtkWidget *vehicle = link_gtk_card_new("VEHICLE", "BMW diagnostic identity");
    ...
    link_gtk_card_append_detail(vehicle, "Manufacturer", "BMW");
    ...
    link_gtk_card_append_note(connection,
        "BMWLINK is GUI-first. LINK owns the common standards flow. BMW-specific claims remain evidence-gated.");
...
static void append_modules(GtkWidget *body, ProductContext *context)
{
    ...
    link_gtk_card_append_note(card,
        "BMW-specific module discovery is added only when its protocol and identity evidence are verified.");
...
static void append_dashboard(GtkWidget *body, ProductContext *context)
{
    ...
    GtkWidget *card = link_gtk_card_new("AT-A-GLANCE", "BMW powertrain dashboard");
    size_t index;
...
{
    GtkWidget *card = link_gtk_card_new("BMWLINK", "Application and adapter information");
    link_gtk_card_append_detail(card, "Version", bmwlink_version());
    link_gtk_card_append_detail(card, "Product", "BMW diagnostics");
    ...
    link_gtk_card_append_detail(card, "Manufacturer layer",
        "BMW-specific knowledge remains evidence-backed");
    gtk_box_append(GTK_BOX(body), card);
}
