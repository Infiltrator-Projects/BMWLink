...
static void vehicle(GtkWidget *b,ProductContext *c){
    GtkWidget *v=link_gtk_card_new("VEHICLE","BMW diagnostic identity");
    ...
    link_gtk_card_append_detail(v,"Manufacturer","BMW");
    ...
    link_gtk_card_append_note(x,"BMWLINK is GUI-first. LINK owns the standards transport and read-only diagnostic flow; manufacturer-specific claims remain evidence-gated.");
...
int main(int argc,char **argv){ProductContext c={0};LinkGtkShellDescriptor d={0};d.app_id="com.github.InfiltratorProjects.BMWLINK";d.window_title="BMWLINK · BMW Diagnostics";d.brand_name="BMWLINK";d.brand_subtitle="BMW · LINK standards diagnostics";d.version=bmwlink_version();d.css=css;d.render_section=render;d.connection_changed=conn;d.diagnostic_changed=diag;d.use_client_side_titlebar=true;d.context=&c;return link_gtk_shell_run(argc,argv,&d);}
